...
your_turn_img = ImageTk.PhotoImage(Image.open("./images/your_turn.png").resize((your_turn_size), Image.ANTIALIAS))
your_turn = Label(window,image=your_turn_img)
#CREATE opponent's turn
opponent_turn_size = [200,100]
opponent_turn_img = ImageTk.PhotoImage(Image.open("./images/opponent_turn.png").resize((your_turn_size), Image.ANTIALIAS))
...
#CREATE timer
timer_size = [200,150]
timer_img = ImageTk.PhotoImage(Image.open("./images/timer.png").resize((timer_size), Image.ANTIALIAS))
...
score_size = [200,125]
score_img = ImageTk.PhotoImage(Image.open("./images/score.png").resize((score_size), Image.ANTIALIAS))
...
#CREATE leave
leave_size = [200,100]
leave_img = ImageTk.PhotoImage(Image.open("./images/leave.png").resize((leave_size), Image.ANTIALIAS))
...
def Leave(event):
    ...
    #leave button status
    leave_img = ImageTk.PhotoImage(Image.open("./images/leave_click.png").resize((leave_size), Image.ANTIALIAS))
...
#CREATE board
board_size = [500,500]
board_img = ImageTk.PhotoImage(Image.open("./images/board.png").resize((board_size), Image.ANTIALIAS))
...
#CREATE submit
submit_size = [100,50]
submit_img = ImageTk.PhotoImage(Image.open("./images/submit.png").resize((submit_size), Image.ANTIALIAS))
...
num_size = [96,96]
nums_locate = [[],[],[],[],[]]
num_img = ImageTk.PhotoImage(Image.open("./images/num.png").resize((num_size), Image.ANTIALIAS))
for i in range(5): #row, x moves, y stays
    y=2+(96+2)*i
    for j in range(5): #column, y moves, x stays
        x = 2+(96+2)*j
        num = Label(board,image=num_img,font=("Comic Sans MS",48),compound=CENTER,fg="orange",text=str(num_array[i][j]))
